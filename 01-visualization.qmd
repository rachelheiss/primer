...
Another use of barplots is to visualize the *joint* distribution of two categorical variables. (See Chapter @probability for the definition of a joint distribution.) Let's look at `race`, as well as `treatment`, in the `trains` data by using the `fill` argument inside the `aes()` aesthetic mapping. Recall the `fill` aesthetic corresponds to the color used to fill the bars.
...
Notice the gray section surrounding the line we plotted. This area is called the confidence interval, which is set to 95% by default. We will learn about confidence intervals in Chapter @probability. You can make the shaded area disappear by adding `se = FALSE` as another argument to `geom_smooth()`.
