...
There are three unknown parameters --- $\beta_0$, $\beta_1$ and $\sigma$ --- just as with the models we used in @three-parameters. Before we get to the five parameter case, it is useful to review this earlier material.
...
As we discussed in @three-parameters, the most common term for a model like this is a "regression." We have "regressed" `lived_after`, our dependent variable, on `election_age`, our (only) independent variable.
...
This is a descriptive model, not a causal model. Remember our motto from Chapter @rubin-causal-model: *No causation without manipulation.* There is no way, for person $i$, to change the years that she has been alive on Election Day. On the day of this election, she is X years old. So, there are not two (or more) potential outcomes. Without more than one potential outcome, there can not be a causal effect.
...
Recall the parallel slopes model that we created in @four-parameters. Another visualization we can create, one that also uses slopes and intercepts for our model, is the interaction model. In this model, the slopes for our two groups **are different**, creating a non-parallel visualization. 
...
After tidying our data, we will extract values and assign sensible names for later use. Note that this is identical to the process from @four-parameters, with the addition of a fourth term (the interaction term):
...
These questions are, purposely, less precise than the ones we tackled in Chapters @two-parameters and @three-parameters, written more in a conversational style. This is how normal people talk.
